...
# ╔═╡ efd594f2-5fc3-4bf3-925c-2f77a424177f
begin
	using Stheno
	using ForwardDiff
	using PDMats
...
# ╔═╡ 895810fc-ae1b-4bc0-a6f2-a9beeeadc579
begin
	σ = 1.0
	l = 1.0
	k = σ * SqExponentialKernel() ∘ ScaleTransform(l)
	#k = compose(SqExponentialKernel(), ScaleTransform(2.0))
end

# ╔═╡ 23e67f82-c48d-4ba7-bf10-b66a45708867
begin
	x1 = [0.1, 0.1, 0.1]
	v1 = [-0.1, -0.1, -0.1]
	k(x1, v1)
...
# ╔═╡ 2baaa910-da76-47d7-89a8-69b9c91ffad5
begin
	∂ₓk(x, v::Vector) = ForwardDiff.gradient(x -> k(x,v), x)
	∂ᵥk(x, v::Vector) = ForwardDiff.gradient(v -> k(x,v), v)
	
	∂ₓ∂ₓk(x, v::Vector) = ForwardDiff.jacobian(
		x -> ForwardDiff.gradient(
			x -> k(x,v)
			, x)
		, x)
	∂ₓ∂ᵥk(x, v::Vector) = ForwardDiff.jacobian(
		v -> ForwardDiff.gradient(
			x -> k(x,v)
			, x)
		, v)
	
	∂ₓ∂ₓ∂ₓk(x, v::Vector) = ForwardDiff.jacobian(
	...
	∂ₓ∂ₓ∂ₓ∂ₓk(x, v::Vector) = ForwardDiff.jacobian(
		x ->ForwardDiff.jacobian(
			x ->ForwardDiff.jacobian(
				x -> ForwardDiff.gradient(
					x -> k(x,v)
...
# ╔═╡ 55c684d9-f8af-4f2a-9f70-f2bd52a79d1d
X = rand(3, 3)

# ╔═╡ aafc2c06-80de-486a-9b4c-1b07a130c8f5
kernelmatrix(k, ColVecs(X))

# ╔═╡ 280d903e-89cd-46ac-8c18-a6b6a3ddc277
begin
	dim = size(X,1)
	num = size(X,2)
end

# ╔═╡ a8dd5f71-d2a3-4a4c-85a2-1b92969c4fbb
KKdiff = zeros(((1+dim)*num, (1+dim)*num))

# ╔═╡ 0f0bf368-0489-4782-b7c9-43c94f7dfaff
KK = zeros(((1)*num, (1)*num))

# ╔═╡ 379748c0-333e-48e6-92ed-91e251bd8b71
for i in 1:num 
	for j in 1:num 
		KK[i, j] = k(X[:,i],X[:,j])
		KKdiff[i, j] = k(X[:,i],X[:,j])
		KKdiff[(num+1)+((i-1)*dim):(num+1)+((i)*dim)-1,j] = ∂ₓk(X[:,i],X[:,j])
		KKdiff[i,(num+1)+((j-1)*dim):(num+1)+((j)*dim)-1] = ∂ᵥk(X[:,i],X[:,j])
		KKdiff[(num+1)+((i-1)*dim):(num+1)+((i)*dim)-1,
			(num+1)+((j-1)*dim):(num+1)+((j)*dim)-1] = ∂ₓ∂ᵥk(X[:,i],X[:,j])
	end
end

# ╔═╡ 7a3c7ca2-cf9b-4b87-bb52-a3bd667803a0
KK

# ╔═╡ c5a13251-19c3-426c-a3fc-d08492c33607
heatmap(KKdiff)

# ╔═╡ 8b975b98-51e9-40aa-a769-77f3ce853be2
KKdiff2 = zeros(((1+dim)*num, (1+dim)*num))

# ╔═╡ 03f159c9-8ef7-43eb-afdc-5987bd723a1f
for i in 1:num 
	for j in 1:num 
		KK[i, j] = k(X[:,i],X[:,j])
		KKdiff2[i, j] = k(X[:,i],X[:,j])
		KKdiff2[(num+1)+((i-1)*dim):(num+1)+((i)*dim)-1,j] = ∂ₓk(X[:,i],X[:,j])
		KKdiff2[i,(num+1)+((j-1)*dim):(num+1)+((j)*dim)-1] = -∂ₓk(X[:,i],X[:,j])
		KKdiff2[(num+1)+((i-1)*dim):(num+1)+((i)*dim)-1,
			(num+1)+((j-1)*dim):(num+1)+((j)*dim)-1] = -∂ₓ∂ₓk(X[:,i],X[:,j])
	end
end

# ╔═╡ 5fbdf4be-9e0e-499b-a8c7-838f9e1e1424
begin
	i = 1
	j = 3
end

# ╔═╡ 98362af1-f346-4252-8304-3b0fffe3b98b
KKdiff[(num+1)+((i-1)*dim):(num+1)+((i)*dim)-1,j] = ∂ₓk(X[:,i],X[:,j])

# ╔═╡ b49ad0ba-8e3e-4253-a5fc-c76ab8fc9333
KKdiff

# ╔═╡ d0d4c363-6437-414c-b055-6ef3bf5de79f
∂ₓk(X[:,i],X[:,j])

# ╔═╡ 243e0843-f989-49e8-80b7-7212704c6f64
(num+1)+((i-1)*dim)
...
git-tree-sha1 = "2787c1cd4853242411cc3da8022f3c23494552a7"
...
git-tree-sha1 = "62e204aced8bb4973eadcfe9b49a3110f88e4829"
...
git-tree-sha1 = "c8f61829e49178507da700bf1f7a328c4bcee4d6"

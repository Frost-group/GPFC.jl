...
# ╔═╡ 945fd85e-2f81-11ee-0212-075dd0685c44
begin
...
# ╔═╡ 8b86985c-4475-4502-a14a-fc3e4d2d571f
...
# ╔═╡ 720edb55-eac3-4e55-83e2-6d7e135fa8e7
...
# ╔═╡ d539f14c-5b2b-4c6b-821b-8f40685962b1
...
# ╔═╡ d90b3e23-4671-4d06-9ebe-29a63f0200b0
...
# ╔═╡ 6a25ecef-bb45-497f-b812-76684f19be4f
...
# ╔═╡ d4e83a86-604e-4570-8dec-433654be348b
begin
...
# ╔═╡ 75922fc6-bbb9-4761-8ccc-c4b7bc327a2e
θ = pi/3

# ╔═╡ fbe586e8-5199-4010-90b1-fc7e939b9e09
begin
	x1 = [1*(cos(θ) + sin(θ)im), 2*(cos(θ) + sin(θ)im), 3*(cos(θ) + sin(θ)im)]
	x2 = [3(cos(θ) + sin(θ)im), 2*(cos(θ) + sin(θ)im), 1*(cos(θ) + sin(θ)im)]
end;

# ╔═╡ 87cfb346-c8dd-4f50-836a-f72da1c1945e
begin
	diff(x, y) = Zygote.gradient( a -> kernel(a, y), x)[1]
	diff1(x, y) = Zygote.gradient( a -> real(kernel(a, real(y))), real(x))[1]
	diff2(x, y) = Zygote.gradient( a -> real(kernel(a, imag(y))), imag(x))[1]
	diff3(x, y) = Zygote.gradient( a -> imag(kernel(a, real(y))), real(x))[1]
	diff4(x, y) = Zygote.gradient( a -> imag(kernel(a, imag(y))), imag(x))[1]
end

# ╔═╡ ff060ca3-6748-44c8-b672-9a422f80aeae
diff1(x1, x2)

# ╔═╡ d958d146-bf63-49fc-8e4b-2f175ca0db14
diff2(x1, x2)

# ╔═╡ 06ad548b-5a83-49dc-84c3-083b509785b2
diff3(x1, x2)

# ╔═╡ a3c78548-54b2-4340-a7c0-1b6cd4d6c254
diff4(x1, x2)

# ╔═╡ e568d9ec-5c96-45a7-a246-621c25a1b27e


# ╔═╡ d6d193cf-b62f-44c0-a850-0167e7575d65
real(diff(x1, x2))

# ╔═╡ 2ab03646-b286-4130-8105-d43480ae464e
imag(diff(x1, x2))

# ╔═╡ 09d348ea-8c4f-4f30-9d01-a6544fa1cefa
gradient(x -> real(log(x)), 1+2im)[1] |> conj
